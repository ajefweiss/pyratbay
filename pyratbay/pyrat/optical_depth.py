...
def optical_depth(pyrat):
    ...
    with get_tracer().log_event("calc ray path"):
    ...
    if pyrat.ex.extfile is not None:
        with get_tracer().log_event("extfile"):
            r = rtop
    ...
    # Calculate the extinction coefficient on the spot:
        ...
        with get_tracer().log_event("tlifile"):
    ...
    with get_tracer().log_event("ext sum"):
        od.ec[rtop:] = (
            + pyrat.ex.ec[rtop:]
            + pyrat.cs.ec[rtop:]
    ...
    if pyrat.cloud.fpatchy is not None:
        with get_tracer().log_event("clear sky spectrum"):
    ...
    if od.rt_path in pc.emission_rt:
        with get_tracer().log_event("pc.emission_rt"):
        ...
        with get_tracer().log_event("pc.transmission_rt"):
